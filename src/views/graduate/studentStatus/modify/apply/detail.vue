                      ...
                      <el-form-item prop="applySign">
                        <template v-if="status != 'view'">
                          <div v-if="ruleForm.applySign">
                            ...
                            <span style="color: #999; margin-left: 20px;" @click="openSign">重新签名</span>
                          </div>
...
const rules = reactive({
  ...
  gender: [{ required: true, message: "请选择异动类别", trigger: "blur" }],
...
onBeforeMount(() => {
  switch (props.status) {
    case "view":
      state.title = "查看学籍异动申请";
      break;
    case "edit":
      state.title = "修改学籍异动申请";
      break;
    case "add":
      state.title = "学籍异动申请";
      break;
