<template>
  <div class="app-container">
    ...
    <div class="table-operator">
      <div class="left">
        <!-- <el-button plain type="primary" icon="CirclePlus" @click="handleUpdate"
          >审核</el-button
        > -->
      </div>
    </div>
  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setModifyTableData(tableState.data);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      } else {
        const rowIndex = getIndexByAttr(tableState.data, "id", row.id);
        graduateStore.removeModifyTableData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const detailChange = (row) => {
  if (detailState.status === "edit") {
    ...
    graduateStore.addModifyTableData(row);
  }
...
const getTableData = () => {
  const searchData = graduateStore.modifyTableData.filter((f) => {
    let flag = true;
