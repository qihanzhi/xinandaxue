  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setInfoTableData(tableState.data);
        getTableData();
        ...
        graduateStore.removeInfoTableData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const detailChange = (row) => {
  if (detailState.status === "edit") {
    ...
    graduateStore.addInfoTableData(row);
  }
...
const getTableData = () => {
  const searchData = graduateStore.infoTableData.filter((f) => {
    let flag = true;
