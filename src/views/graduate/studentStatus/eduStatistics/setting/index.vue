<template>
  <div class="app-container">
    <p class="container-title">教育事业综合统计调查表自定义统计维度设置</p>
  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setEduSettingData(tableState.data);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      } else {
        const rowIndex = getIndexByAttr(tableState.data, "id", row.id);
        graduateStore.removeEduSettingData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const detailChange = (row) => {
  if (detailState.status === "edit") {
    ...
    graduateStore.addEduSettingData(row);
  }
...
const getTableData = () => {
  const searchData = graduateStore.eduSettingData.filter((f) => {
    let flag = true;
