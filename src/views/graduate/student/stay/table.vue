      ...
      >
        ...
        >
          <div class="search-grid">
            ...
            <el-form-item label="留校类型:" prop="transactionType">
              <el-select
                clearable
  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setStayData(tableState.data);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      } else {
        const rowIndex = getIndexByAttr(tableState.data, "id", row.id);
        graduateStore.removeStayData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const getTableData = () => {
  const searchData = graduateStore.stayData.filter((f) => {
    let flag = true;
