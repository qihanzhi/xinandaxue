        ...
        >
          <div class="search-grid">
            <el-form-item label="辅导员姓名:" prop="name">
              <el-input v-model="queryParams.name" placeholder="请输入辅导员姓名" />
            </el-form-item>
  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setCoachData(tableState.data);
        getTableData();
        ...
        graduateStore.removeCoachData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const detailChange = (row) => {
  if (detailState.status === "edit") {
    ...
    graduateStore.addCoachData(row);
  }
...
const getTableData = () => {
  const searchData = graduateStore.coachData.filter((f) => {
    let flag = true;
