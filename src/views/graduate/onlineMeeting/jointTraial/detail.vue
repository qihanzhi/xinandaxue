  ...
  >
    ...
    <template #default>
      <div class="flow-container">
        <el-steps class="mb-4" :space="200" :active="flowState.step" simple>
          ...
          <el-step title="会审表决" :icon="Picture" />
              ...
              <el-row :gutter="35">
                <el-col :span="24">
                  <el-form-item :label="'学位类型'" :prop="'degreeType'">
                    <template v-if="status != 'view' && false">
                      <el-select
      ...
      <div class="content-container" v-if="flowState.step === 2">
        ...
        >
          <div class="vote-container">
            <div class="con1" v-if="!state.isVote">
              <div class="label">进行投票表决</div>
              <div class="con">
                <el-radio-group v-model="ruleForm.voteResult" :disabled="status == 'view'">
                  <el-radio value="通过">通过</el-radio>
                  <el-radio value="不通过">不通过</el-radio>
                </el-radio-group>
              </div>
            </div>
            <div class="con2" v-else>
              <div class="label">当前投票进度</div>
              <div class="con">
                <el-progress
                  :percentage="(ruleForm.curVoteNum / ruleForm.voteTotal) * 100"
                />
              </div>
            </div>
          </div>
      ...
      <div class="content-container" v-if="flowState.step === 3">
        <div
          ...
          "
        >
          <div v-if="!state.isEnd">等待所有委员表决完成！</div>
          <div class="end-container" v-else>
            <div class="label">当前投票结果</div>
            <div class="con">
              <ResultSatistic :ruleForm="ruleForm"></ResultSatistic>
            </div>
          </div>
        </div>
      </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" v-if="flowState.step > 0" @click="preChange">上一步</el-button>
          <el-button @click="cancel">取 消</el-button>
          <el-button @click="submitForm">确 定</el-button>

          <el-button type="primary" v-if="flowState.step < 3" @click="nextChange">下一步</el-button>
...
import ResultSatistic from "./statistic.vue";
...
onBeforeMount(() => {
  ...
  // 是否已投票
  state.isVote = props.formData.voteResult;

  // 投票是否已结束
  if (
    props.formData.voteResult &&
    props.formData.voteTotal == props.formData.curVoteNum
  ) {
    flowState.step = 3;
    state.isEnd = true;
  }
