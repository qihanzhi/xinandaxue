<template>
  <table class="calendar-one table table-bordered calendar-table">
    <thead>
...
$(function () {
  ...
  for (var j = 0; j < 27; j++) {
    ...
    var b = $(".calendar-one tbody");
    b.append("<tr><th scope='row'>" + m + "月" + "</th></tr>");
    $(".calendar-one tbody tr")
      .last()
  ...
  function find() {
    var startRow, endRow;
    var rows = $(".calendar-one tbody tr th").length;
    // console.log(rows);
    // console.log($(".calendar-one tbody tr th").eq(1).html());

    for (var i = 0; i < rows; i++) {
      // console.log($(".calendar-one tbody tr th").eq(1).html());
      // console.log(i);
      if (
        $(".calendar-one tbody tr th").eq(i).html() ==
        $(".calendar-one tbody tr th")
          .eq(i + 1)
          .html()
      ) {
        startRow = i; //开始节点
        ...
        for (var j = i; j < rows; j++) {
          if (
            $(".calendar-one tbody tr th").eq(j).html() !=
            $(".calendar-one tbody tr th")
              .eq(j + 1)
              .html()
          ) {
  ...
  function hebing(startRow, endRow) {
    ...
    for (var i = startRow + 1; i <= endRow; i++) {
      $(".calendar-one tbody tr th").eq(i).attr("id", i);
    }
    $(".calendar-one tbody tr th")
      .eq(startRow)
  ...
  // 单元格点击
  $(".calendar-one tbody tr .day-box").on("click", (e) => {
