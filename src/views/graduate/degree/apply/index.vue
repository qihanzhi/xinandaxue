  ...
  proxy.$modal
    ...
    .then(() => {
      if (row instanceof PointerEvent) {
        ...
        graduateStore.setDegreeApplyData(tableState.data);
        getTableData();
        ...
        graduateStore.removeDegreeApplyData(rowIndex);
        getTableData();
        proxy.$modal.msgSuccess("删除成功");
      }
    })
...
const detailChange = (row) => {
  if (detailState.status === "edit") {
    ...
    graduateStore.addDegreeApplyData(row);
  }
...
const getTableData = () => {
  const searchData = graduateStore.degreeApplyData.filter((f) => {
    let flag = true;
